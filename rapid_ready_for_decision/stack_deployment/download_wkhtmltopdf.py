...
# This URL points to the latest (as of this writing) build of the wkhtmltopdf binary compiled with Amazon Linux V2.
# Update this URL if the binary needs to be updated - make sure it's compiled for Amazon Linux
...
def download_wkhtmltopdf_main() -> None:
    ...
    filename = binary_request.headers["Content-Disposition"].split(
        "filename="
    )[-1]
